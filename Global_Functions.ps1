...
function Update-Windows {
    ...
    try {
        ...
        if (-not (Get-WUServiceManager | Where-Object ServiceID -eq "7971f918-a847-4430-9279-4a52d1efe18d")) {
            ...
            Write-Host "Registered Microsoft Update service source."
        }
    } catch {
    ...
    $logPath = "$logDir\Updates_$(Get-Date -Format 'yyyy-MM-dd_HH-mm-ss').txt"
    $formattedUpdates | Set-Content -Path $logPath
    ...
    Write-Host "Windows Update process complete. Log saved to $logPath"
}
